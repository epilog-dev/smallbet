...
- **Vercel AI SDK 7** (`ai`), structured output via `generateText`/`streamText` + `Output.object(zodSchema)`. **Provider = Gemini for now** (`@ai-sdk/google`, your free `GOOGLE_GENERATIVE_AI_API_KEY`), default model `gemini-3.6-flash` (bump via `AI_MODEL` env, e.g. to the newest Flash). `@ai-sdk/anthropic` is also wired so setting `AI_PROVIDER=anthropic` + `ANTHROPIC_API_KEY` switches to Claude with no code change. A deterministic `MockGenerator` sits behind the same interface for tests and keyless dev.
